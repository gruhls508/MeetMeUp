...
- (void)viewDidLoad
{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:
     ...
     {
         self.meetUpArray = [[NSJSONSerialization JSONObjectWithData:data options:0 error:nil]objectForKey:@"results"];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
     ...
     ^(NSURLResponse *rsp, NSData *data, NSError *connectionError)
    {
        ...
        vc.detailEventArray = [[NSJSONSerialization JSONObjectWithData:data options:0 error:nil]objectForKey:@"results"];
